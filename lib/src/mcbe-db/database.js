...
function matchLike(value, pattern, escapeChar) {
    let out = "^";
...
function compare(clause, left) {
    ...
    if (op === "BETWEEN") {
        ...
        if (typeof left === "string" && typeof a === "string" && typeof b === "string")
            return left >= a && left <= b;
        return false;
    }
    if (op === "IS_NULL")
...
function resolveColumnValue(row, ref, outerRow) {
    if (ref.table) {
    ...
    if (outerRow) {
        if (ref.table) {
            ...
            if (outerDirect !== undefined)
                return outerDirect;
        }
        if (ref.column in outerRow)
            return outerRow[ref.column];
    }
    return row[ref.column];
}
// WHEREは OR で分割されたグループ（中は AND 条件）として評価する。
function rowMatchesWhereBasic(row, where) {
    if (!where)
        return true;
    ...
    if (where.type === "not")
        return !rowMatchesWhereBasic(row, where.node);
    if (where.type === "and")
        return rowMatchesWhereBasic(row, where.left) && rowMatchesWhereBasic(row, where.right);
    if (where.type === "or")
        return rowMatchesWhereBasic(row, where.left) || rowMatchesWhereBasic(row, where.right);
    throw new Error("Subqueries are not supported in this context.");
}
function joinMatches(row, on) {
    if (!on)
        return true;
    if (on.type === "clause") {
...
function projectReturningRows(rows, spec) {
    if (spec.columns === "*")
        return rows.map((r) => ({ ...r }));
    const columns = spec.columns;
    return rows.map((r) => projectRow(r, columns));
}
function isOrderByPosition(ref) {
    return typeof ref.type === "string" && ref.type === "position";
}
const checkCache = new Map();
function validateChecks(table, row) {
...
function compareForSortWithNulls(a, b, nulls) {
    if (nulls) {
...
function rowMatchesUniqueConstraint(row, other, columns) {
...
function inferColumnDefsFromInsert(columns) {
...
export class WorldSqlDatabase {
    ...
    load() {
        ...
        try {
            ...
            if (parsed.v !== 1)
                throw new Error(`Unsupported DB snapshot version: ${String(parsed.v)}`);
    ...
    import(data) {
        try {
            ...
            if (snapshot.v !== 1)
                throw new Error(`Unsupported DB snapshot version: ${String(snapshot.v)}`);
    ...
    transaction(fn) {
        ...
        try {
            ...
            try {
                this.snapshot = JSON.parse(before);
            }
            catch {
                this.snapshot = emptySnapshot();
            }
    ...
    getTableFromContext(name, ctes) {
        if (ctes && ctes[name])
            return ctes[name];
        return this.getTable(name);
    }
    validateValueType(col, value) {
        if (value === null || value === undefined)
            return;
        switch (col.type) {
            case "TEXT":
            ...
            case "REAL":
                if (typeof value !== "number")
                    throw new Error(`Column ${col.name} expects REAL.`);
                return;
            case "BOOLEAN":
                if (typeof value !== "boolean")
                    throw new Error(`Column ${col.name} expects BOOLEAN.`);
                return;
            case "JSON":
                return;
        }
    }
    primaryKeyColumn(def) {
        if (!def.primaryKey)
            return undefined;
    ...
    ensureTable(name, columns) {
        ...
        const table = { def, rows: [], autoInc: 1 };
        this.snapshot.tables[name] = table;
        this.dirty = true;
        return table;
    }
    createTable(table, columns, primaryKey, ifNotExists, uniqueConstraints = [], checks = []) {
        ...
        if (pk && !columns.some((c) => c.name === pk))
        ...
        const def = { name: table, columns, primaryKey: pk, autoIncrement, uniqueConstraints, checks };
    ...
    createTableAsSelect(table, select, ifNotExists, ctes) {
        if (this.snapshot.tables[table]) {
            if (ifNotExists)
                return;
            throw new Error(`Table already exists: ${table}`);
    ...
    runSelectLike(query, outerRow, ctes) {
        if (query.type === "set")
            return this.executeSetQuery(query, outerRow, ctes);
        return this.select(query, outerRow, ctes);
    }
    executeSetQuery(query, outerRow, ctes) {
    ...
    createSingleRowTable(name, row) {
        const columns = Object.keys(row).map((col) => ({ name: col, type: "JSON" }));
        return {
            def: {
                name,
                columns,
    ...
    replaceTriggerRefs(statement, newRow, oldRow) {
        let out = "";
        let i = 0;
        let quote = null;
        while (i < statement.length) {
            ...
            if (slice === "NEW." || slice === "OLD.") {
                ...
                const start = i + 4;
                let j = start;
                while (j < statement.length && /[A-Za-z0-9_]/.test(statement[j]))
                    j++;
    ...
    fireTriggers(timing, event, table, newRow, oldRow) {
        ...
        for (const trigger of triggers) {
            ...
            const ctes = {};
            if (newRow)
                ctes.NEW = this.createSingleRowTable("NEW", newRow);
            if (oldRow)
                ctes.OLD = this.createSingleRowTable("OLD", oldRow);
    ...
    alterTableAddColumn(tableName, column) {
        ...
        if (column.default !== undefined) {
            this.validateValueType(column, column.default);
        }
    ...
    alterTableRenameColumn(tableName, from, to) {
        const table = this.getTable(tableName);
        if (!table)
            throw new Error(`Table not found: ${tableName}`);
        ...
        if (!col)
            throw new Error(`Column not found: ${from}`);
        col.name = to;
        if (table.def.primaryKey === from)
            table.def.primaryKey = to;
        table.def.uniqueConstraints = (table.def.uniqueConstraints ?? []).map((cols) => cols.map((c) => (c === from ? to : c)));
        for (const row of table.rows) {
        ...
        for (const existing of table.rows) {
            ...
            if (!conflict) {
                for (const cols of table.def.uniqueConstraints ?? []) {
                    if (!cols.every((c) => Object.prototype.hasOwnProperty.call(row, c)))
                        continue;
    ...
    insertRow(tableName, columns, values, orAction) {
        ...
        if (pk) {
            ...
            if (pkCol && (pkCol.type === "INTEGER" || pkCol.type === "REAL")) {
                ...
                if (typeof row[pk] === "number" && row[pk] >= table.autoInc) {
                    table.autoInc = Math.floor(row[pk]) + 1;
                }
                this.lastInsertId = row[pk];
            }
        }
            ...
            if (orAction === "REPLACE") {
                ...
                for (const col of table.def.columns) {
                    ...
                    if (Object.prototype.hasOwnProperty.call(row, col.name)) {
                        ...
                        if (dup)
                            throw new Error(`Duplicate UNIQUE value for ${col.name}: ${String(row[col.name])}`);
                    }
                }
                for (const cols of table.def.uniqueConstraints ?? []) {
                    if (!cols.every((c) => Object.prototype.hasOwnProperty.call(row, c)))
                    ...
                    if (dup)
                        throw new Error(`Duplicate UNIQUE constraint for (${cols.join(", ")}).`);
                }
            }
        }
    ...
    rowWithPrefixes(tableRef, source, fillUndefined = false) {
        const out = {};
        const aliases = [tableRef.alias, tableRef.name].filter(Boolean);
    ...
    select(query, outerRow, ctes) {
        const baseTable = this.getTableFromContext(query.from.name, ctes);
        if (!baseTable)
            return [];
        let current = baseTable.rows.map((r) => this.rowWithPrefixes(query.from, r));
        ...
        for (const row of current) {
            if (!this.rowMatchesWhere(row, query.where, outerRow, ctes))
                continue;
            filtered.push(row);
        }
        const hasAggregates = query.columns !== "*" && query.columns.some((c) => c.expr.type === "aggregate");
        ...
        if (query.columns === "*") {
            ...
            if (hasSubquery)
                throw new Error("Scalar subqueries are not supported with GROUP BY/aggregates.");
            const selectColumns = query.columns;
            ...
            if (filtered.length === 0 && hasAggregates && groupBy.length === 0) {
                ...
                for (const row of filtered) {
                    const key = stableStringify(groupBy.map((g) => resolveColumnValue(row, g)));
            ...
            const aggregateValue = (rows, expr) => {
                ...
                switch (expr.func) {
                    ...
                    case "MIN": {
                        let min = null;
                        for (const v of values) {
                            if (min === null || compareForSort(v, min) < 0)
                                min = v;
                        }
                        return min;
                    }
                    case "MAX": {
                        let max = null;
                        for (const v of values) {
                            if (max === null || compareForSort(v, max) > 0)
                                max = v;
                        }
                        return max;
                    }
            ...
            for (const [_, rows] of groups) {
                ...
                if (!this.rowMatchesWhere(aggregated, query.having, outerRow, ctes))
                    continue;
        ...
        if (query.orderBy && query.orderBy.length > 0) {
            const specs = query.orderBy.map((spec) => {
                if (isOrderByPosition(spec.ref)) {
                    ...
                    return { ...spec, ref: { column: key } };
                }
                return spec;
            });
            out = [...out].sort((ra, rb) => {
                for (const spec of specs) {
                    const av = resolveColumnValue(ra, spec.ref);
                    const bv = resolveColumnValue(rb, spec.ref);
    ...
    projectSelectRow(row, columns, outerRow, ctes) {
        const projected = {};
        for (const c of columns) {
            if (c.expr.type === "column") {
    ...
    evalScalarSubquery(query, outerRow, ctes) {
        ...
        const keys = Object.keys(row);
        if (keys.length !== 1)
            throw new Error("Scalar subquery must return exactly one column.");
        return firstRowValue(row);
    }
    evalExpr(expr, row, outerRow, ctes) {
        if (expr.type === "literal")
            return expr.value;
        if (expr.type === "column")
            return resolveColumnValue(row, expr.ref, outerRow);
        if (expr.type === "subquery")
            return this.evalScalarSubquery(expr.query, outerRow ?? row, ctes);
        const args = expr.args.map((arg) => this.evalExpr(arg, row, outerRow, ctes));
        return this.evalFunction(expr.name, args);
    }
    evalFunction(name, args) {
        const func = name.toUpperCase();
        const requireArgs = (min, max) => {
        ...
        const toNumber = (value) => {
            if (typeof value !== "number" || !Number.isFinite(value))
                throw new Error(`Function ${func} expects numeric arguments.`);
            return value;
        };
        const toString = (value) => {
            if (typeof value !== "string")
                throw new Error(`Function ${func} expects string arguments.`);
            return value;
        };
        switch (func) {
            case "LENGTH":
    ...
    rowMatchesWhere(row, where, outerRow, ctes) {
        if (!where)
            return true;
        if (where.type === "clause") {
        ...
        if (where.type === "not")
            return !this.rowMatchesWhere(row, where.node, outerRow, ctes);
        if (where.type === "and") {
    ...
    update(tableName, set, where, limit, ctes) {
        const table = this.getTable(tableName);
        if (!table)
        ...
        for (const row of table.rows) {
            if (changed >= max)
                break;
            if (!this.rowMatchesWhere(row, where, undefined, ctes))
                continue;
            ...
            for (const col of table.def.columns) {
                ...
                if (Object.prototype.hasOwnProperty.call(set, col.name)) {
                    this.validateValueType(col, value);
                }
            }
            ...
            if (pk && Object.prototype.hasOwnProperty.call(set, pk)) {
                ...
                if (dup)
                    throw new Error(`Duplicate primary key for table ${tableName}: ${String(nextPk)}`);
            }
            for (const col of table.def.columns) {
                if (!col.unique)
                    continue;
                ...
                if (dup)
                    throw new Error(`Duplicate UNIQUE value for ${col.name}: ${String(nextValue)}`);
            }
            for (const cols of table.def.uniqueConstraints ?? []) {
                const affects = cols.some((c) => Object.prototype.hasOwnProperty.call(set, c));
                if (!affects)
                    continue;
                if (!cols.every((c) => Object.prototype.hasOwnProperty.call(nextRow, c)))
                    continue;
    ...
    delete(tableName, where, limit, ctes) {
        const table = this.getTable(tableName);
        if (!table)
            return { changes: 0, rows: [] };
        const kept = [];
        ...
        switch (query.type) {
            case "createTable":
                if (query.asSelect) {
                    ...
                    this.createTable(query.table, query.columns, query.primaryKey, query.ifNotExists, query.uniqueConstraints ?? [], query.checks ?? []);
                }
                this.lastChanges = 0;
                if (this.autoSave)
                    this.save();
