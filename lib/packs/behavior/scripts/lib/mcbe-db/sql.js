...
function tokenize(sql) {
    ...
    let line = 1;
    let col = 1;
    const push = (type, value, startIndex, startLine, startCol) => tokens.push({ type, value, index: startIndex, line: startLine, col: startCol });
    ...
    while (i < s.length) {
        ...
        if (numberStart) {
            ...
            if (s[j] === "0" && (s[j + 1] === "x" || s[j + 1] === "X")) {
                ...
                if (hexCount === 0)
                    throw new Error("Invalid hex number literal.");
            ...
            while (j < s.length) {
                ...
                if (c === ".") {
                    if (dotSeen || expSeen)
                        throw new Error("Invalid number literal: unexpected dot.");
                    dotSeen = true;
                    j++;
                    continue;
                }
                if (c === "e" || c === "E") {
                    if (expSeen)
                        throw new Error("Invalid number literal: duplicate exponent.");
                    expSeen = true;
                    j++;
                    if (s[j] === "+" || s[j] === "-")
                        j++;
                    ...
                    if (expDigits === 0)
                        throw new Error("Invalid number literal: missing exponent digits.");
                    break;
                }
                break;
            }
            if (digitCount === 0)
                throw new Error("Invalid number literal: missing digits.");
            if (s[j - 1] === ".")
                throw new Error("Invalid number literal: trailing dot.");
...
class Cursor {
    ...
    expect(type, value) {
        const t = this.next();
        if (t.type !== type)
            throw new Error(`Expected ${type} but got ${t.type} (${t.value}) at ${t.line}:${t.col}`);
    ...
    parseValue() {
        ...
        if (t.type === "placeholder") {
            if (this.params.length === 0)
                throw new Error("Missing SQL params for placeholder.");
...
function assertEof(cursor) {
    if (cursor.matchSymbol(";"))
        cursor.next();
    if (cursor.peek().type !== "eof")
        throw new Error("Unexpected token after SQL statement.");
}
function parseColumnType(typeIdent) {
...
function parseCheckExpression(cursor, sql) {
    ...
    while (depth > 0) {
        ...
        if (t.type === "symbol" && t.value === ")")
            depth--;
        if (t.type === "eof")
            throw new Error("Unterminated CHECK expression.");
        end = t.index;
    }
    return sql.slice(start, end).trim();
}
function parseColumnDef(cursor, sql) {
    ...
    const parseClauseNode = () => {
        ...
        if (opToken.type === "ident") {
            ...
            if (kw === "NOT") {
                ...
                if (next !== "IN" && next !== "LIKE" && next !== "BETWEEN")
                    throw new Error(`Unsupported WHERE operator: NOT ${next}`);
                op = next;
            }
            else if (kw === "IS") {
                cursor.next();
                ...
                if (nullKw !== "NULL")
                    throw new Error("IS expects NULL.");
            ...
            if (op === "LIKE") {
                ...
                if (typeof value !== "string")
                    throw new Error("LIKE expects a string.");
                let escape;
                if (cursor.matchIdent("ESCAPE")) {
                    cursor.next();
                    const esc = cursor.parseValue();
                    if (typeof esc !== "string" || esc.length !== 1)
                        throw new Error("ESCAPE expects a single character.");
                    escape = esc;
                }
                const clause = { column, op, value, escape };
                if (kw === "NOT")
                    return { type: "not", node: { type: "clause", clause } };
            ...
            if (op === "IN") {
                ...
                if (cursor.matchIdent("SELECT")) {
                    ...
                    const query = parseSelectOrSetAfterSelect(cursor);
                    cursor.expect("symbol", ")");
                    const node = { type: "inSubquery", column, query };
                    if (kw === "NOT")
                ...
                if (!cursor.matchSymbol(")")) {
                    while (true) {
                        ...
                        if (!isSqlPrimitive(value))
                            throw new Error("IN values must be primitive.");
                        list.push(value);
                        if (cursor.matchSymbol(",")) {
                            cursor.next();
                            continue;
                ...
                cursor.expect("symbol", ")");
                const clause = { column, op, value: list };
                if (kw === "NOT")
                    return { type: "not", node: { type: "clause", clause } };
                return { type: "clause", clause };
            }
            if (op === "BETWEEN") {
                ...
                if (!isSqlPrimitive(first))
                    throw new Error("BETWEEN expects primitive values.");
                cursor.expect("ident", "AND");
                const second = cursor.parseValue();
                if (!isSqlPrimitive(second))
                    throw new Error("BETWEEN expects primitive values.");
                const clause = { column, op, value: [first, second] };
                if (kw === "NOT")
                    return { type: "not", node: { type: "clause", clause } };
                return { type: "clause", clause };
            }
...
function parseWhere(cursor) {
    if (!cursor.matchIdent("WHERE"))
        return undefined;
    cursor.next();
    return parseCondition(cursor);
}
export function parseWhereExpression(expression) {
    const cursor = new Cursor(tokenize(`WHERE ${expression}`), []);
    const where = parseWhere(cursor);
    if (!where)
        throw new Error("Failed to parse WHERE expression.");
    assertEof(cursor);
    return where;
}
function parseTableRef(cursor) {
    const name = cursor.parseIdent();
    let alias;
...
function parseJoinCondition(cursor) {
    const parseClauseNode = () => {
        const left = cursor.parseColumnRef();
        if (!cursor.matchOp("="))
            throw new Error("JOIN ON supports only '=' comparisons.");
        cursor.next();
...
function parseNonNegativeInt(token) {
    const n = Number(token.value);
    if (!Number.isInteger(n) || n < 0)
        throw new Error("Value must be a non-negative integer.");
    return n;
}
function parseLimitOffset(cursor) {
    if (!cursor.matchIdent("LIMIT"))
        return {};
    cursor.next();
...
function parseFetchLimit(cursor) {
    if (!cursor.matchIdent("FETCH"))
        return undefined;
    cursor.next();
...
function parseOrderBySpec(cursor) {
    let ref;
    if (cursor.peek().type === "number") {
        const pos = parseNonNegativeInt(cursor.next());
        if (pos === 0)
            throw new Error("ORDER BY position must be >= 1.");
...
function parseSelectAfterSelect(cursor) {
    let distinct = false;
    let topLimit;
    while (true) {
            ...
            const expr = (() => {
                if (cursor.peek().type === "ident") {
                    ...
                    if (["COUNT", "SUM", "AVG", "MIN", "MAX"].includes(ident) && cursor.peekNext().value === "(") {
                        const func = cursor.parseIdent().toUpperCase();
                        cursor.expect("symbol", "(");
                ...
                if (parsed.type === "column")
                    return { type: "column", ref: parsed.ref };
                if (parsed.type === "subquery")
                    return { type: "subquery", query: parsed.query };
    ...
    if (fetchLimit !== undefined) {
        if (limit !== undefined)
            throw new Error("FETCH cannot be combined with LIMIT.");
        limit = fetchLimit;
    }
    if (topLimit !== undefined) {
        if (limit !== undefined)
            throw new Error("TOP cannot be combined with LIMIT/FETCH.");
        limit = topLimit;
    }
    const q = {
        type: "select",
...
function parseReturning(cursor) {
    if (!cursor.matchIdent("RETURNING"))
        return undefined;
    cursor.next();
    if (cursor.matchSymbol("*")) {
        cursor.next();
...
function parseSelectOrSetAfterSelect(cursor) {
    let left = parseSelectAfterSelect(cursor);
    while (true) {
        let op;
        if (cursor.matchIdent("UNION"))
            op = "UNION";
        else if (cursor.matchIdent("INTERSECT"))
            op = "INTERSECT";
        else if (cursor.matchIdent("EXCEPT"))
            op = "EXCEPT";
        if (!op)
            break;
        cursor.next();
...
function parseWithClause(cursor) {
...
export function parseSql(sql, params = []) {
    ...
    if (cursor.matchIdent("CREATE")) {
        ...
        if (cursor.matchIdent("TRIGGER")) {
            ...
            if (timing !== "BEFORE" && timing !== "AFTER")
                throw new Error("TRIGGER timing must be BEFORE or AFTER.");
            ...
            while (true) {
                ...
                if (t.type === "eof")
                    throw new Error("Unterminated trigger body.");
            }
            ...
            const q = { type: "createTrigger", trigger };
            assertEof(cursor);
            return q;
        }
        ...
        if (cursor.matchIdent("AS")) {
            ...
            const q = { type: "createTable", table, columns: [], ifNotExists, asSelect };
            assertEof(cursor);
            return q;
        }
        cursor.expect("symbol", "(");
        ...
        while (true) {
            ...
            if (cursor.matchIdent("PRIMARY")) {
                ...
                if (hadConstraintName)
                    throw new Error("CONSTRAINT must be followed by PRIMARY/UNIQUE/CHECK.");
    ...
    if (cursor.matchIdent("DROP")) {
        ...
        if (cursor.matchIdent("TRIGGER")) {
            ...
            const name = cursor.parseIdent();
            const q = { type: "dropTrigger", name, ifExists };
            assertEof(cursor);
            return q;
        }
        cursor.expect("ident", "TABLE");
        ...
        const table = cursor.parseIdent();
        const q = { type: "dropTable", table, ifExists };
        assertEof(cursor);
        return q;
    }
    if (cursor.matchIdent("ALTER")) {
        ...
        const q = { type: "alterTable", table, action };
        assertEof(cursor);
        return q;
    }
    ...
    if (cursor.matchIdent("DESCRIBE") || cursor.matchIdent("DESC")) {
        cursor.next();
        const table = cursor.parseIdent();
        const q = { type: "describeTable", table };
        assertEof(cursor);
        return q;
    }
    if (cursor.matchIdent("INSERT")) {
        ...
        const q = { type: "insert", table, columns, values, select, defaultValues, or: orAction, returning };
        assertEof(cursor);
        return q;
    }
    if (cursor.matchIdent("SELECT")) {
    ...
    if (cursor.matchIdent("UPDATE")) {
        ...
        const returning = parseReturning(cursor);
        const q = { type: "update", table, set, where, limit, returning };
        assertEof(cursor);
        return q;
    }
    if (cursor.matchIdent("DELETE")) {
        ...
        const { limit } = parseLimitOffset(cursor);
        const returning = parseReturning(cursor);
        const q = { type: "delete", table, where, limit, returning };
        assertEof(cursor);
        return q;
    }
